...
- (void)setDisplayData:(TPWeiboDataModel *)dataModel
{
    ...
    [self.textView setTextArray:dataModel.textArray];
...
- (void)setDisplayData:(TPWeiboDataModel *)dataModel
{
    ...
    self.textView.frame = textViewFrame;
    // 加载图片:从缓存取或者异步加载
    if(dataModel.thumbnailImage)
        self.picImageView.image = dataModel.thumbnailImage;
    else
        [[TPImageDownloadCenter sharedInstance] loadImageWithURL:dataModel.thumbnailImageURL Type:TPWeiboImageTypeThumbnail ViewDelegate:self ModelDelegate:dataModel ProgressDelegate:nil];
...
- (void)setDisplayData:(TPWeiboDataModel *)dataModel
{
    ...
    const CGRect repostTextViewFrame = CGRectMake(nameLabelFrame.origin.x, nameLabelFrame.origin.y + nameLabelFrame.size.height + 5 , dataModel.repostTextSize.width, dataModel.repostTextSize.height);
    ...
    [self.repostTextView setTextArray:dataModel.repostTextArray];
